...
export class FX {
  ...
  fairy(x, y) {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      this.add({ x: x + (Math.random() - 0.5) * 40, y: y + (Math.random() - 0.5) * 40,
        vx: Math.cos(a) * 2, vy: Math.sin(a) * 2 - 1,
        sz: 3 + Math.random() * 5, c: c[randInt(0, 3)], dk: 0.018, g: -0.02, sh: 's', glow: true })
    }
  }
  fighting(x, y) {
    const c = ['#b45309', '#d97706', '#f59e0b', '#fef3c7']
    this.burst(x, y, 25, c, { sp: 5, sz: 4, dk: 0.03 })
    this.fire(x, y)
  }
  heal(x, y) {
